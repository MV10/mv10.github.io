...
While I don't intend to get into the nitty gritty details of OpenGL, working with textures is frankly a pain in the ass. You have texture _handles_ which is how you'd work with data in most normal APIs. But OpenGL requires you to go a step further and also assign textures to texture units (TUs), which are apparently the numeric identifiers of memory slots where textures are actually stored. You'd think an API could figure those things out transparently, but 20 years ago some sort of backwards compatibility decision was made that probably seemed like a good idea at the time, and today we're still paying the price.
